...
export const GifPreview: React.FC<GifPreviewProps> = ({
  ...
  return (
    <figure className={`my-6 group/gif ${className}`}>
      {/* Container frame */}
      <div 
        className="relative overflow-hidden rounded-xl border border-hairline-outline bg-surface-container hover:border-secondary/50 transition-all duration-200 shadow-md group"
      >
        {/* Quick action top-right: Expand to lightbox */}
        ...
        <div 
          ...
          className="relative flex items-center justify-center min-h-[140px] max-h-[460px] bg-canvas-obsidian/40 cursor-pointer overflow-hidden"
        >
          ...
          {hasError ? (
            <div className="p-8 text-center space-y-2">
              ...
              <p className="font-mono text-xs text-text-disabled truncate max-w-sm">{src}</p>
            </div>
          ) : (
            <>
              {/* Active animated GIF */}
          ...
          {!hasError && (
            <div 
              className={`absolute inset-0 bg-canvas-obsidian/40 flex items-center justify-center backdrop-blur-xs transition-opacity duration-200 pointer-events-none ${
                isPlaying 
                  ? 'opacity-0 group-hover:opacity-100' 
                  : 'opacity-100 bg-canvas-obsidian/30 hover:bg-canvas-obsidian/40'
              }`}
            >
              <div className="p-3.5 rounded-full bg-secondary/90 text-canvas-obsidian shadow-xl flex items-center justify-center transform group-hover:scale-110 transition-transform">
                {isPlaying ? (
        ...
        {/* Bottom Bar: centered caption with muted text color */}
        {(caption || alt) && (
          <div className="px-3.5 py-2.5 bg-surface-elevated/80 border-t border-hairline-subtle flex items-center justify-center text-center">
            <span className="text-xs font-mono text-text-muted truncate max-w-full font-normal">
              {caption || alt}
            </span>
          </div>
        )}
          ...
          >
            ...
            <div className="flex items-center gap-1.5 sm:gap-2">
              ...
              <div className="hidden sm:flex items-center border border-hairline-outline rounded-md bg-surface-elevated divide-x divide-hairline-subtle">
                ...
                <span className="px-2 text-xs font-mono text-text-muted select-none">
